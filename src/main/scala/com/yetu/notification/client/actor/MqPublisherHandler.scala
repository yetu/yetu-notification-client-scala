 ...
 */
class MqPublisherHandler(producer: ActorRef, exchangeParameters: ExchangeParameters) extends Actor with ActorLogging {
...
//      producer ? Publish(Config.RABBITMQ_EXCHANGE, message.key, message.payload.toString.getBytes)
...
object MqPublisherHandler{
  def props(producer: ActorRef, exchangeParameters:ExchangeParameters): Props = Props(new MqPublisherHandler(producer,exchangeParameters))
}